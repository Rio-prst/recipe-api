...
class TagRepository {
  ...
  async attachToRecipe(recipeId: number, tagId: number): Promise<Tag> {
    await this.db.query(`INSERT INTO recipe_tags (recipe_id, tag_id) VALUES ($1, $2) ON CONFLICT DO NOTHING;`, [recipeId, tagId]);
  ...
  async findRecipeBySlug(slug: string, page: number, limit: number, difficulty?: string): Promise<{ tag: Tag; data: Recipe[]; total: number }> {
